...
static void ggml_sycl_op_mul_mat_tiled(
    ...
    if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        launch_gemm_f16_f32_tiled(stream, src1_f16, (const sycl::half*)src0_dd_i, dst_dd_i, N, M, K, 1.0f, 0.0f, K, K, ldc, false);
...
void ggml_sycl_mul_mat(ggml_backend_sycl_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    if (ctx.force_graph_compatible) {
        if (xmx_gemm_available(ctx.stream())) {
            ...
            if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) xmx_types = true;
            else if (src0->type == GGML_TYPE_F16 && src1->type == GGML_TYPE_F16) xmx_types = true;
...
static void k_count_experts(
    ...
    row_dst_index[global_id] = dpct::atomic_fetch_add<sycl::access::address_space::generic_space>(
        &expert_counts[expert_id], 1);
}


static void k_scan_experts(
...
static void ggml_sycl_mul_mat_id_tiled(ggml_backend_sycl_context & ctx, ggml_tensor *dst) {
    ...
    stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<1>(1, 1), [=](sycl::nd_item<1> item) {
             k_scan_experts(counts_ptr, offsets_ptr, n_experts, item);
        });
    });
...
void ggml_sycl_mul_mat_id(ggml_backend_sycl_context & ctx,
                                 ggml_tensor *dst) try {
    if (ctx.force_graph_compatible) {

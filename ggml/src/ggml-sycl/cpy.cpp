...
static void cpy_1_f32_bf16(const char * cxi, char * cdsti) {
    const float * xi   = (const float *) cxi;
    bfloat16 *    dsti = (bfloat16 *) cdsti;

    dsti[0] = fp32_to_bf16(*xi);
}

static void cpy_1_bf16_bf16(const char * cxi, char * cdsti) {
    const bfloat16 * xi   = (const bfloat16 *) cxi;
    bfloat16 *       dsti = (bfloat16 *) cdsti;
...
static void cpy_1_f16_bf16(const char * cxi, char * cdsti) {
    ...
    dsti[0] = fp32_to_bf16(static_cast<float>(xi[0]));
}
...
static void cpy_1_bf16_f16(const char * cxi, char * cdsti) {
    ...
    dsti[0] = sycl::vec<float, 1>(bf16_to_fp32(xi[0])).convert<sycl::half, sycl::rounding_mode::automatic>()[0];
}
#endif

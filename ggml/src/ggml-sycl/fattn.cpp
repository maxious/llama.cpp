...
inline bool ggml_sycl_flash_attn_has_xmx(sycl::device device) {
    ...
    if (!device.has(sycl::aspect::ext_intel_gpu_eu_simd_width) || !device.has(sycl::aspect::ext_intel_matrix)) {
        return false;
    }
...
inline bool ggml_sycl_flash_attn_xmx_allow_head(int64_t head_size) {
    ...
    if (!allowlist_checked) {
        ...
        if (env == nullptr) {
            allowlist = { 64, 80, 96, 112, 128, 256, 512, 576 };
...
inline bool can_use_direct_loading(const ggml_tensor * Q,
                                   const ggml_tensor * K,
                                   const ggml_tensor * V,
                                   const ggml_tensor * dst) {
    ...
    if (Q->nb[0] != q_elem_size) {
        return false;
    }
    if (K->nb[0] != k_elem_size) {
        return false;
    }
    if (V->nb[0] != v_elem_size) {
        return false;
    }
    if (dst->nb[0] != o_elem_size) {
        return false;
    }
...
template <int64_t DQK, int64_t DV> void ggml_sycl_op_flash_attn_2(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
            ...
            stream->submit([&](sycl::handler & cgh) {
                cgh.parallel_for(sycl::range<1>((n_elements + 255) / 256 * 256), [=](sycl::item<1> it) {
                    ...
                    if (idx < n_elements) {
                        ...
                        Q_d_f32_alloc[head * N * DQK + idx] =
                            static_cast<float>(Q_d[head * q_head_stride_f16 + row * q_row_stride_f16 + col]);
                    }
                });
            });
        }
            ...
            stream->submit([&](sycl::handler & cgh) {
                cgh.parallel_for(sycl::range<1>((n_elements + 255) / 256 * 256), [=](sycl::item<1> it) {
                    ...
                    if (idx < n_elements) {
                        const int64_t row = idx / DQK;
                        const int64_t col = idx % DQK;
                        K_d_f32_alloc[head * N_kv * DQK + idx] =
                            static_cast<float>(K_d[head * k_head_stride_f16 + row * k_row_stride_f16 + col]);
                    }
                });
            });
        }
                    ...
                    cgh.parallel_for(sycl::range<1>((n_elements + 255) / 256 * 256), [=](sycl::item<1> it) {
                        ...
                        if (idx < n_elements) {
                            ...
                            V_d_f32_alloc[head * N_kv * DV + idx] =
                                static_cast<float>(V_d[head * v_head_stride_f16 + row * v_row_stride_f16 + col]);
                        }
                    });
                });
            }
    ...
    stream->memset(l_d, 0, N * n_heads * sizeof(float));
    {
        ...
        stream->submit([&](sycl::handler & cgh) {
            cgh.parallel_for(sycl::range<1>(total_lm), [=](sycl::id<1> idx) { m_d[idx] = -1.0e20f; });
        });
    }
    ...
    stream->submit([&](sycl::handler & cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
            ...
            // Q @ K^T
            flash_attn_mul_mat_QK_kernel<DQK>(it, Q_block, q_row_stride, K_block, k_row_stride, s_loc, (ptrdiff_t) Bc,
                                              Br, Bc);

            it.barrier(sycl::access::fence_space::local_space);

            // Softmax with causal masking and optional sliding window
            flash_attn_softmax_kernel(it, s_loc, p_loc, m_loc, l_loc, Br, Bc, l_row, m_row,
            ...
            // P @ V
            flash_attn_mul_mat_PV_kernel<DV>(it, p_loc, (ptrdiff_t) Bc, V_block, v_row_stride, O_block, o_seq_stride,
                                             Br, Bc);
...
void ggml_sycl_op_flash_attn(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    auto         try_xmx  = [&]() -> bool {
        ...
        bool direct_supported_shape =
            (actual_d == actual_dv && actual_d == padded_d) || (actual_d == 576 && actual_dv == 512);
        ...
        try {
            ...
            if (n_splits > 1) {
                ...
                if (shape_supported) {
                    ...
                    const size_t shmem_bytes = ggml_sycl_flash_attn_xmx_shmem_bytes(actual_d, actual_dv, 32, 32, false);
                    if (!ggml_sycl_flash_attn_xmx_shmem_ok(device, shmem_bytes)) {
                        GGML_SYCL_DEBUG(
            ...
            if (actual_d != actual_dv && padded_d > 0 && padded_dv > 0) {
                ...
                if (v_from_k_view && actual_d == 576 && actual_dv == 512) {
                    ...
                    GGML_SYCL_DEBUG("ggml_sycl: Using XMX MLA flash attention (576/512)\n");
                    GGML_SYCL_ITT_FATTN_XMX(576);
                ...
                const size_t shmem_bytes = ggml_sycl_flash_attn_xmx_shmem_bytes(padded_d, padded_dv, 32, 32, false);
                if (!ggml_sycl_flash_attn_xmx_shmem_ok(device, shmem_bytes)) {
                    GGML_SYCL_DEBUG(
    ...
    if (DQK <= 128 && DV <= 128 && DQK == DV) {
        ...
        if (mask_supported) {
            ...
            GGML_SYCL_DEBUG("Fused strides: Q_seq=%ld Q_head=%ld K_seq=%ld K_head=%ld V_seq=%ld V_head=%ld\n",
                            strides.q_stride_seq, strides.q_stride_head, strides.k_stride_seq, strides.k_stride_head,
...
mkl_fallback:
    ...
    if (fallback_dqk != fallback_dv) {
        GGML_ABORT(
            "ggml_sycl: Flash attention fallback requires DQK==DV, got DQK=%ld DV=%ld. "
            "Enable XMX or oneMKL for MLA head sizes (e.g., 576/512).\n",

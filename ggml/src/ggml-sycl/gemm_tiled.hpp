...
inline void gemm_tiled_kernel_general(sycl::nd_item<2>               it,
            ...
            if (flat_idx < A_TILE_SIZE) {
                ...
                if (global_row < M && global_k < K) {
                    if constexpr (transpose_A) {
                        val = (float) A[global_k * lda + global_row];
                    } else {
                        val = (float) A[global_row * lda + global_k];
                    }
                }
...
#pragma unroll
        for (int tn = 0; tn < TN; ++tn) {
            ...
            if (beta == 0.0f) {
                ...
                C[c_idx] = alpha * acc[tm][tn] + beta * (float) C[c_idx];
            }
        }
    }
}
...
inline void gemm_tiled_kernel(sycl::nd_item<2>               it,
                              sycl::local_accessor<float, 1> tile_A,
                              sycl::local_accessor<float, 1> tile_B,
                              ...
                              const int   ldb,
                              const int   ldc) {
    gemm_tiled_kernel_general<float, float, float, BM, BN, BK, TM, TN, transpose_A, transpose_B>(
        it, tile_A, tile_B, A, B, C, M, N, K, alpha, beta, lda, ldb, ldc);
}

template <typename T_A, typename T_B, typename T_C, bool transpose_A = false, bool transpose_B = true>
inline void launch_gemm_tiled_general(sycl::queue * stream,
                                      const T_A *   A,
                                      const T_B *   B,
                                      T_C *         C,
                                      const int     M,
    ...
    stream->submit([&](sycl::handler & cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
            gemm_tiled_kernel_general<T_A, T_B, T_C, BM, BN, BK, TM, TN, transpose_A, transpose_B>(
                it, tile_A, tile_B, A, B, C, M, N, K, alpha, beta, lda, ldb, ldc);
        });
    });
}
...
inline void launch_gemm_tiled(sycl::queue * stream,
                              const float * A,
                              const float * B,
                              float *       C,
                              const int     M,
    ...
    stream->submit([&](sycl::handler & cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
            gemm_tiled_kernel<BM, BN, BK, TM, TN, false, transpose_B>(it, tile_A, tile_B, A, B, C, M, N, K, alpha, beta,
                                                                      lda, ldb, ldc);
        });
    });
}
...
inline void launch_gemm_tiled_A_transposed(sycl::queue * stream,
                                           const float * A,
                                           const float * B,
                                           float *       C,
                                           const int     M,
    ...
    stream->submit([&](sycl::handler & cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
            gemm_tiled_kernel<BM, BN, BK, TM, TN, true, false>(it, tile_A, tile_B, A, B, C, M, N, K, alpha, beta, lda,
                                                               ldb, ldc);
        });
    });
}
...
template <bool transpose_B = true>
inline void launch_gemm_tiled_batched(sycl::queue * stream,
                                      const float * A,
                                      const float * B,
                                      float *       C,
                                      const int     M,
...
template <int BM, int BN, int BK, int TM, int TN, bool transpose_A = false, bool transpose_B = true>
inline void gemm_tiled_kernel_indirect(sycl::nd_item<2>               it,
                                       sycl::local_accessor<float, 1> tile_A,
                                       sycl::local_accessor<float, 1> tile_B,
                                       const float * __restrict__ A,
                                       const float * __restrict__ B,
...
template <bool transpose_B = true>
inline void launch_gemm_tiled_indirect(sycl::queue * stream,
                                       const float * A,
                                       const float * B,
                                       float *       C,
                                       const int *   M_ptr,
    ...
    stream->submit([&](sycl::handler & cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
            gemm_tiled_kernel_indirect<BM, BN, BK, TM, TN, false, transpose_B>(
                it, tile_A, tile_B, A, B, C, M_ptr, offset_ptr, N, K, alpha, beta, lda, ldb, ldc);
        });
    });
}

template <int BM, int BN, int BK, int TM, int TN, bool transpose_A = false, bool transpose_B = true>
inline void gemm_tiled_kernel_indirect_f32_f16(sycl::nd_item<2>               it,
                                               sycl::local_accessor<float, 1> tile_A,
                                               sycl::local_accessor<float, 1> tile_B,
                                               const float * __restrict__ A,
...
template <bool transpose_B = true>
inline void launch_gemm_tiled_indirect_f32_f16(sycl::queue *      stream,
                                               const float *      A,
                                               const sycl::half * B,
                                               float *            C,
                                               const int *        M_ptr,
...
template <bool transpose_B = true>
inline void launch_gemm_tiled_batched_indirect(sycl::queue *  stream,
                                               const float ** A,
                                               const float ** B,
                                               float **       C,
                                               const int      M,
    ...
    // Use 3D grid: (M tiles, N tiles, batch)
    // Note: sycl::range<3>(dim0, dim1, dim2) -> get_group(0) is dim0?
    // Standard mapping: range(z, y, x).
    // In launch_gemm_tiled_batched: global(batch, grid_m * WG_M, grid_n * WG_N)
    // b = it.get_group(0). So batch is outer dim.
    sycl::range<3> global(batch, grid_m * WG_M, grid_n * WG_N);
    sycl::range<3> local(1, WG_M, WG_N);

...
inline void gemm_f16_f32_tiled_kernel(sycl::nd_item<2>               it,
...
inline void launch_gemm_f16_f32_tiled(sycl::queue * stream,
                                      const void *  A_src,
                                      const void *  B_src,
                                      void *        C_dst,
                                      const int     M,
    ...
    stream->submit([&](sycl::handler & cgh) {
        ...
        if (transpose_B) {
            cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
                gemm_f16_f32_tiled_kernel<BM, BN, BK, TM, TN, false, true>(it, tile_A, tile_B, A, B, C, M, N, K, alpha,
                                                                           beta, lda, ldb, ldc);
            });
        } else {
            cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
                gemm_f16_f32_tiled_kernel<BM, BN, BK, TM, TN, false, false>(it, tile_A, tile_B, A, B, C, M, N, K, alpha,
                                                                            beta, lda, ldb, ldc);
            });
        }
    });
...
inline void launch_gemm_f16_f32_tiled_A_transposed(sycl::queue * stream,
                                                   const void *  A_src,
                                                   const void *  B_src,
                                                   void *        C_dst,
                                                   const int     M,
    ...
    stream->submit([&](sycl::handler & cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
            gemm_f16_f32_tiled_kernel<BM, BN, BK, TM, TN, true, false>(it, tile_A, tile_B, A, B, C, M, N, K, alpha,
                                                                       beta, lda, ldb, ldc);
        });
    });
}
...
inline void gemm_f16_f32_tiled_kernel_batched_indirect(sycl::nd_item<3>               it,
                                                       sycl::local_accessor<float, 1> tile_A,
                                                       sycl::local_accessor<float, 1> tile_B,
                                                       const sycl::half * const * __restrict__ A,
                                                       const sycl::half * const * __restrict__ B,
    ...
    const sycl::half * A_b = A[b];
    const sycl::half * B_b = B[b];
    float *            C_b = C[b];
...
#pragma unroll
        for (int load = 0; load < A_LOADS_PER_THREAD; ++load) {
            ...
            if (flat_idx < A_TILE_SIZE) {
                ...
                float val = 0.0f;
                if (global_row < M && global_k < K) {
                    sycl::half h = A_b[global_row * lda + global_k];
                    val          = static_cast<float>(h);
                }
                tile_A[tile_m * BK + tile_k] = val;
            }
        ...
        for (int load = 0; load < B_LOADS_PER_THREAD; ++load) {
            ...
            if (flat_idx < B_TILE_SIZE) {
                ...
                if (global_col < N && global_k < K) {
                    sycl::half h;
                    if constexpr (transpose_B) {
                        h = B_b[global_k * ldb + global_col];
                    } else {
                        h = B_b[global_col * ldb + global_k];
                    }
                    val = static_cast<float>(h);
                }
                tile_B[tile_n * BK + tile_k] = val;
...
inline void launch_gemm_f16_f32_tiled_batched_indirect(sycl::queue *              stream,
                                                       const sycl::half * const * A,
                                                       const sycl::half * const * B,
                                                       float * const *            C,
                                                       const int                  M,
                                                       const int                  N,
                                                       const int                  K,
    ...
    stream->submit([&](sycl::handler & cgh) {
        ...
        if constexpr (transpose_B) {
            cgh.parallel_for(sycl::nd_range<3>(global, local), [=](sycl::nd_item<3> it) {
                gemm_f16_f32_tiled_kernel_batched_indirect<BM, BN, BK, TM, TN, true>(it, tile_A, tile_B, A, B, C, M, N,
                                                                                     K, alpha, beta, lda, ldb, ldc);
            });
        } else {
            cgh.parallel_for(sycl::nd_range<3>(global, local), [=](sycl::nd_item<3> it) {
                gemm_f16_f32_tiled_kernel_batched_indirect<BM, BN, BK, TM, TN, false>(it, tile_A, tile_B, A, B, C, M, N,
                                                                                      K, alpha, beta, lda, ldb, ldc);
            });
        }
    });

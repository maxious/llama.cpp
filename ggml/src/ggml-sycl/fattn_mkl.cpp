...
void ggml_sycl_op_flash_attn_mkl(ggml_backend_sycl_context & ctx, ggml_tensor * dst) {
    ...
    if (first_call) {
        fprintf(stderr,
                "ggml_sycl: oneMKL flash attention ACTIVE: N=%ld N_kv=%ld n_heads=%ld n_splits=%ld kv_per_split=%ld\n",
    ...
    // Allocate or reuse buffer for Q (pooled)
    const size_t Q_bytes = N * DQK * n_heads * sizeof(float);
    float *      Q_d_f32 = ctx.fattn_buffers->get_Q(Q_bytes, stream);
    if (q_is_f16) {
    ...
    // Allocate or reuse buffer for K (pooled)
    const size_t K_bytes = N_kv * DQK * n_kv_heads * sizeof(float);
    float *      K_d_f32 = ctx.fattn_buffers->get_K(K_bytes, stream);
    if (k_is_f16) {
...
#if defined(GGML_SYCL_GRAPH) && SYCL_EXT_ONEAPI_ASYNC_MEMORY_ALLOC
        if (use_async_mem) {
            V_d_f32_alloc = (float *) syclex::async_malloc(*stream, sycl::usm::alloc::device,
                                                           N_kv * DV * n_kv_heads * sizeof(float));
        } else
#endif
        {
            V_d_f32_alloc = ctx.fattn_buffers->get_V(N_kv * DV * n_kv_heads * sizeof(float), stream);
        }
        stream->submit([&](sycl::handler & cgh) {
    ...
    for (int64_t split = 0; split < n_splits; ++split) {
        ...
        if (gqa_ratio == 1) {
            launch_gemm_tiled_batched<true>(stream, Q_d_f32, K_d_f32 + kv_start * DQK, S_d + kv_start, N, kv_chunk_size,
                                            DQK, scale, 0.0f, n_heads, lda_q, lda_k, lda_s, stride_q, stride_k,
                                            stride_s);
        } else {
            for (int64_t kv_head = 0; kv_head < n_kv_heads; ++kv_head) {
                ...
                launch_gemm_tiled_batched<true>(
                    stream, Q_d_f32 + q_head_start * stride_q, K_d_f32 + kv_head * stride_k + kv_start * DQK,
                    S_d + q_head_start * stride_s + kv_start, N, kv_chunk_size, DQK, scale, 0.0f, gqa_ratio, lda_q,
                    lda_k, lda_s, stride_q, 0, stride_s);  // B stride = 0: same K matrix for all batches
            }
        }
        ...
        if (gqa_ratio == 1) {
            launch_gemm_tiled_batched<false>(stream, S_d + kv_start, V_d_f32 + kv_start * DV, O_ptr, N, DV,
                                             kv_chunk_size, 1.0f, 0.0f, n_heads, lda_s, lda_v, ldc_o, stride_s,
                                             stride_v, stride_o);
        } else {
            for (int64_t kv_head = 0; kv_head < n_kv_heads; ++kv_head) {
                const int64_t q_head_start = kv_head * gqa_ratio;
                launch_gemm_tiled_batched<false>(
                    stream, S_d + q_head_start * stride_s + kv_start, V_d_f32 + kv_head * stride_v + kv_start * DV,
                    O_ptr + q_head_start * stride_o, N, DV, kv_chunk_size, 1.0f, 0.0f, gqa_ratio, lda_s, lda_v, ldc_o,
    ...
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(sycl::nd_range<2>(sycl::range<2>(n_heads * N, DV), sycl::range<2>(1, DV)),
                         [=](sycl::nd_item<2> it) {
                             ...
                             flash_attn_combine_splits_kernel<DV>(it, partials, O_d, n_splits, n_heads, N, partial_size,
                                                                  o_nb1, o_nb2, sinks_d);
                         });
    });

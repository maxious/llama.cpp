...
//
// Uses joint_matrix FP16 instructions for the two heavy matmuls:
//   S = Q * K^T  (score computation)
//   O = P * V    (output accumulation)
// Online softmax and rescaling done in FP32.
//
// Tile dimensions:
//   BQ = 16  (query rows per work-group)
...
//   Sub-groups 0-1 handle the two TM=8 row halves of BQ for Q*K^T
//   All sub-groups cooperate on data loading
//
...
static constexpr int FA_BQ = 16;   // query rows per work-group (2 * FA_TM)
...
static void flash_attn_xmx_kernel(
    ...
    const sycl::nd_item<3> & item_ct1)
{
    ...
    constexpr int PV_scratch_size = 2 * FA_TM * FA_TN;  // for 2 sub-groups
    ...
    for (int kv_start = 0; kv_start < ne11; kv_start += FA_BK) {
        ...
        // BQ = 16 = 2 * TM -> 2 SG rows
        // BK = 16 = 1 * TN -> 1 SG col
        // Need 2 SGs for score computation. Use sg_id 0 and 2 (or 0 and 1).
        // Let's use: sg 0 handles rows 0-7, sg 1 handles rows 8-15
        // Each computes a [TM x TN] = [8 x 16] block of scores.
        ...
        if (sg_id < 2) {
            ...
            for (int dk = 0; dk < DKQ / FA_TK; ++dk) {
                syclmx::joint_matrix<sycl::sub_group, sycl::half, syclmx::use::a, FA_TM, FA_TK, syclmx::layout::row_major> mat_q;
                syclmx::joint_matrix<sycl::sub_group, sycl::half, syclmx::use::b, FA_TK, FA_TN, syclmx::layout::row_major> mat_kt;

                // Load Q slice: rows [sg_m_off, sg_m_off+TM), cols [dk*TK, (dk+1)*TK)
                syclmx::joint_matrix_load(sg, mat_q,
                ...
                syclmx::joint_matrix_load(sg, mat_kt,
                    sycl::address_space_cast<sycl::access::address_space::local_space, sycl::access::decorated::no>(
        ...
        if (sg_id < 2) {
            ...
            for (int dt = 0; dt < DV_TILES; ++dt) {
                syclmx::joint_matrix<sycl::sub_group, sycl::half, syclmx::use::a, FA_TM, FA_TK, syclmx::layout::row_major> mat_p;
                syclmx::joint_matrix<sycl::sub_group, sycl::half, syclmx::use::b, FA_TK, FA_TN, syclmx::layout::row_major> mat_v;
                ...
                syclmx::joint_matrix_load(sg, mat_p,
                    sycl::address_space_cast<sycl::access::address_space::local_space, sycl::access::decorated::no>(
                        S_slm + sg_m_off * FA_BK),
                    FA_BK);

                syclmx::joint_matrix_load(sg, mat_v,
                    sycl::address_space_cast<sycl::access::address_space::local_space, sycl::access::decorated::no>(
                        V_slm + dt * FA_TN),
                    DV_PAD);
                ...
                syclmx::joint_matrix_store(sg, mat_pv,
                    sycl::address_space_cast<sycl::access::address_space::local_space, sycl::access::decorated::no>(pv_local),
...
bool ggml_sycl_fattn_xmx_supported(ggml_backend_sycl_context & ctx, const ggml_tensor * dst) {
    ...
    if (g_ggml_sycl_disable_xmx) return false;
    ...
    if (DKQ != DV) return false;
    if (DKQ % FA_TK != 0) return false;
    if (DV  % FA_TN != 0) return false;

    // Only for prompt processing (many queries)
    if (Q->ne[1] <= 32) return false;

    // Check device supports matrix extensions
    dpct::queue_ptr stream = ctx.stream();
    if (!stream->get_device().has(sycl::aspect::ext_intel_matrix)) return false;
    ...
    stream->submit([&](sycl::handler & cgh) {                                                  \
        cgh.parallel_for(                                                                      \
            ...
            [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {          \
                flash_attn_xmx_kernel<D, D>(                                                   \
                    Q_data, K_data, V_data, mask_data, dst_data,                               \
                    scale,                                                                     \
                    ne00, ne01_val, ne02_val, ne03_val,                                        \
                    nb01_val, nb02_val, nb03_val,                                              \
                    ne10, ne11_val, ne12_val,                                                  \
                    nb11, nb12, nb13,                                                          \
                    nb21, nb22, nb23,                                                          \
                    ne31, nb31_val, nb33_val,                                                  \
                    item_ct1);                                                                 \
            });                                                                                \
    })

    switch (DKQ) {
        case  64: LAUNCH_FATTN_XMX(64);  break;
        case  80: LAUNCH_FATTN_XMX(80);  break;
        case  96: LAUNCH_FATTN_XMX(96);  break;
        case 112: LAUNCH_FATTN_XMX(112); break;
        case 128: LAUNCH_FATTN_XMX(128); break;
        case 256: LAUNCH_FATTN_XMX(256); break;
        default:

...
// 3. Computes Q5_K dot product with expert weights
// 4. Writes to dst directly via strides (no unpacking)

constexpr int Q5_K_FUSED_N_DST = 8;
constexpr int Q5_K_FUSED_N_SG  = 4;

template <int SG_SIZE, int N_DST, int N_SG>
inline void gemm_q5_K_f32_sg_kernel_direct(
    sycl::nd_item<2>                   it,
    ...
    const block_q5_K * B = (const block_q5_K *)(B_base + expert_id * expert_stride);
    const int nb = K / QK_K;
...
// Launch direct-ids Q5_K GEMM - zero pack/unpack overhead
// Reads ids tensor in-kernel, accesses src1 and dst directly via strides.
inline void launch_gemm_direct_ids_q5_K(sycl::queue *  stream,
    ...
    constexpr int SG_SIZE = Q5_K_SG_SIZE;
    constexpr int N_DST   = Q5_K_FUSED_N_DST;
    constexpr int N_SG    = Q5_K_FUSED_N_SG;
    constexpr int WG_SIZE = N_SG * SG_SIZE;
    ...
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(
            ...
            [=](sycl::nd_item<2> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
                gemm_q5_K_f32_sg_kernel_direct<SG_SIZE, N_DST, N_SG>(

...
// directly in-kernel. Modeled after the Q8_0 direct-ids kernel.
//
...
// 2. Reads src1 row directly via strides (F32, no packing)
// 3. Computes MXFP4 dot product with expert weights
// 4. Writes to dst directly via strides (no unpacking)

constexpr int MXFP4_FUSED_N_DST = 8;
constexpr int MXFP4_FUSED_N_SG  = 4;

template <int SG_SIZE, int N_DST, int N_SG, int NB>
inline void gemm_mxfp4_f32_sg_kernel_direct(
    sycl::nd_item<2>           it,
    ...
    const block_mxfp4 * B = (const block_mxfp4 *)(B_base + expert_id * expert_stride);
...
// Launch direct-ids MXFP4 GEMM - zero pack/unpack overhead
// Reads ids tensor in-kernel, accesses src1 and dst directly via strides.
inline void launch_gemm_direct_ids_mxfp4(sycl::queue *  stream,
    ...
    constexpr int SG_SIZE = MXFP4_SG_SIZE;
    constexpr int N_DST   = MXFP4_FUSED_N_DST;
    constexpr int N_SG    = MXFP4_FUSED_N_SG;
    constexpr int NB_Q    = MXFP4_NB;
    constexpr int WG_SIZE = N_SG * SG_SIZE;
    ...
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(
            sycl::nd_range<2>(global, local),
            [=](sycl::nd_item<2> it) [[sycl::reqd_sub_group_size(SG_SIZE)]] {
                gemm_mxfp4_f32_sg_kernel_direct<SG_SIZE, N_DST, N_SG, NB_Q>(

...
// Tiled Graph-Compatible GEMM for MXFP4 inputs -> F32 output
// Fuses dequantization into the GEMM kernel to save memory bandwidth.
//
// A: M x K (MXFP4), B: N x K (F16/BF16/F32), C: M x N (F32)
// A is quantized: block_mxfp4 (32 elements per block).
// Layout: Row-major blocks. A[row] consists of K/32 blocks.
//
...
inline void gemm_mxfp4_f32_tiled_kernel(
    ...
    const block_mxfp4 * __restrict__ A,
    const sycl::half * __restrict__ B, // B is converted to F16 before call
    ...
    // Pre-calculate K block divisors/shifts if QK_MXFP4 is power of 2 (32)
    // k / 32 -> k >> 5
    // k % 32 -> k & 31
    ...
    for (int k_tile = 0; k_tile < num_k_tiles; ++k_tile) {
        ...
        // Load A tile [BM x BK] from MXFP4 and dequantize to float
        #pragma unroll
        for (int load = 0; load < A_LOADS_PER_THREAD; ++load) {
            ...
            if (flat_idx < A_TILE_SIZE) {
                ...
                if (global_row < M && global_k < K) {
                    ...
                    // A is [M][K]. Blocks are [M][K/32].
                    // lda should be K (elements) -> lda/32 (blocks)
                    const int block_k = global_k / QK_MXFP4;
                    const int elem_k = global_k % QK_MXFP4;
                    
                    // Index in block_mxfp4 array
                    // A points to block_mxfp4*.
                    // offset = global_row * (lda / QK_MXFP4) + block_k
                    const int block_idx = global_row * (lda / QK_MXFP4) + block_k;
                    
                    const block_mxfp4 b = A[block_idx];
                    
                    // Dequantize
                    // Get scale E8M0 -> float
        ...
        // Load B tile [BN x BK] from F16 and convert to float
        #pragma unroll
...
// Launch function for MXFP4->F32 GEMM (non-batched)
inline void launch_gemm_mxfp4_f32_tiled(
    ...
    const block_mxfp4 * A = static_cast<const block_mxfp4*>(A_src);
    ...
    stream->submit([&](sycl::handler& cgh) {
        ...
        if (transpose_B) {
            cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
                gemm_mxfp4_f32_tiled_kernel<BM, BN, BK, TM, TN, true>(
                    it, tile_A, tile_B, A, B, C, M, N, K, alpha, beta, lda, ldb, ldc
                );
            });
        } else {
            cgh.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> it) {
                gemm_mxfp4_f32_tiled_kernel<BM, BN, BK, TM, TN, false>(
                    it, tile_A, tile_B, A, B, C, M, N, K, alpha, beta, lda, ldb, ldc
                );
            });
        }

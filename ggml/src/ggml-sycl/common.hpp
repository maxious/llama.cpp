...
struct ggml_backend_sycl_context {
    ...
    queue_ptr graph_exec_stream() {
        if (!_graph_exec_queue) {
            _graph_exec_queue = std::make_unique<sycl::queue>(dpct::get_device(device).create_in_order_queue());
        }
    ...
    struct fattn_bufs {
        ...
        float * ensure_dst_tmp(ggml_sycl_pool & p, size_t n) {
            if (!dst_tmp) {
                dst_tmp = std::make_unique<ggml_sycl_pool_alloc<float>>(p);
            }
            if (dst_tmp->actual_size < n * sizeof(float)) {
                dst_tmp->realloc(n);
            }
            return dst_tmp->get();
        }
        ...
        sycl::float2 * ensure_dst_tmp_meta(ggml_sycl_pool & p, size_t n) {
            if (!dst_tmp_meta) {
                dst_tmp_meta = std::make_unique<ggml_sycl_pool_alloc<sycl::float2>>(p);
            }
            if (dst_tmp_meta->actual_size < n * sizeof(sycl::float2)) {
                dst_tmp_meta->realloc(n);
            }
...
template <int width> static __dpct_inline__ int warp_reduce_sum(int x) {
    return sycl::reduce_over_group(sycl::ext::oneapi::this_work_item::get_sub_group(), x, sycl::plus<>());
}

/* use WARP_SIZE or WARP_32_SIZE*/
template <int width> static __dpct_inline__ float warp_reduce_sum(float x) {
#pragma unroll
...
/* use WARP_SIZE or WARP_32_SIZE*/
template <int width> static __dpct_inline__ float warp_reduce_sum(float x, const sycl::nd_item<3> & item_ct1) {
#pragma unroll
    for (int offset = width / 2; offset > 0; offset >>= 1) {
        x += dpct::permute_sub_group_by_xor(item_ct1.get_sub_group(), x, offset);
    }
    return x;
}

/* use WARP_SIZE or WARP_32_SIZE*/
template <int width> static __dpct_inline__ sycl::float2 warp_reduce_sum(sycl::float2 a) {
#pragma unroll
    for (int offset = width / 2; offset > 0; offset >>= 1) {
        a.x() +=
...
/* use WARP_SIZE or WARP_32_SIZE*/
template <int width> static __dpct_inline__ sycl::half2 warp_reduce_sum(sycl::half2 a) {
#pragma unroll
    for (int offset = width / 2; offset > 0; offset >>= 1) {
...
/* use WARP_SIZE or WARP_32_SIZE*/
template <int width> static __dpct_inline__ int warp_reduce_all(int x) {
    if (width == ggml_sycl_get_physical_warp_size()) {
        return sycl::all_of_group(
            sycl::ext::oneapi::this_work_item::get_sub_group(),
            (~0xffffffff & (0x1 << sycl::ext::oneapi::this_work_item::get_sub_group().get_local_linear_id())) || x);
    } else {
#pragma unroll
        for (int offset = width / 2; offset > 0; offset >>= 1) {
            x = dpct::permute_sub_group_by_xor(sycl::ext::oneapi::this_work_item::get_sub_group(), x, offset, width) &&
...
/* use WARP_SIZE or WARP_32_SIZE*/
template <int width> static __dpct_inline__ int warp_reduce_any(int x) {
    if (width == ggml_sycl_get_physical_warp_size()) {
...
/* use WARP_SIZE or WARP_32_SIZE*/
template <int width> static __dpct_inline__ float warp_reduce_max(float x) {
#pragma unroll
    for (int offset = width / 2; offset > 0; offset >>= 1) {
        x = sycl::fmax(
            x, dpct::permute_sub_group_by_xor(sycl::ext::oneapi::this_work_item::get_sub_group(), x, offset, width));
...
template <int nbytes, int alignment = 0> static __dpct_inline__ void ggml_sycl_memcpy_1(void * dst, const void * src) {
...
template <typename T> sycl::half2 __dpct_inline__ make_half2(T x, T y) {
...
template <typename T> sycl::float2 __dpct_inline__ make_float2(T x, T y) {
...
template <int n> struct ggml_sycl_unroll {
    template <typename Func, typename... Args> void operator()(const Func & f, Args... args) const {
        f(n - 1, args...);
        ggml_sycl_unroll<n - 1>{}(f, args...);
    }
};

template <> struct ggml_sycl_unroll<1> {
    template <typename Func, typename... Args> void operator()(const Func & f, Args... args) const { f(0, args...); }
};
...
template <block_reduce_method method_t, typename T, int warp_size> struct block_reduce_policy;

template <typename T, typename... Ts> inline constexpr bool is_any = (std::is_same_v<T, Ts> || ...);

template <typename...> inline constexpr bool ggml_sycl_dependent_false_v = false;

...
// Reorder kernels request QK_WARP_SIZE (32) subgroups but use WARP_SIZE for work distribution
...
static void mul_mat_vec_q_reorder(const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    ...
    // Use QK_WARP_SIZE for work distribution (matches actual subgroup size)
    constexpr int blocks_per_subgroup         = ceil_div(block_traits::vdr_mmvq * QK_WARP_SIZE, block_traits::qi);
...
#pragma unroll
        for (int elem = 0; elem < block_elements_per_subgroup; elem += QK_WARP_SIZE) {
            // x block quant index when casting the quants to int
            const int iqs = elem + block_traits::vdr_mmvq * (nd_item.get_local_id(2) % block_elements_per_subgroup);
...
#pragma unroll
    for (int mask = QK_WARP_SIZE / 2; mask > 0; mask >>= 1) {
        partial_sum += dpct::permute_sub_group_by_xor(nd_item.get_sub_group(), partial_sum, mask);
    }
...
static void mul_mat_vec_q_K_reorder(const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    ...
    const int     blocks_per_row              = ncols / block_traits::qk;
    // Use QK_WARP_SIZE (32) for K-quant types
    constexpr int blocks_per_subgroup         = ceil_div(block_traits::vdr_mmvq * QK_WARP_SIZE, block_traits::qi);
    constexpr int block_elements_per_subgroup = block_traits::qi / block_traits::vdr_mmvq;
    const int     nblocks                     = nrows * (ncols / block_traits::qk);
...
static void mul_mat_vec_q(const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    ...
    constexpr int blocks_per_warp = (vdr * QK_WARP_SIZE + qi - 1) / qi;  // Ensuring blocks_per_warp > 0
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row; i += blocks_per_warp) {
        ...
        for (size_t elem = 0; elem < qi / vdr; elem += QK_WARP_SIZE) {
...
// Specialized version for K-quant types that need 32-wide sub-groups
template <int qk, int qi, typename block_q_t, int vdr, vec_dot_q_sycl_t vec_dot_q_sycl>
static void mul_mat_vec_q_K(const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
                           const int ncols, const int nrows, const sycl::nd_item<3> & item_ct1) {
    const int row = item_ct1.get_group(2) * item_ct1.get_local_range(1) + item_ct1.get_local_id(1);
    ...
    const int     blocks_per_row  = ncols / qk;
    constexpr int blocks_per_warp = (vdr * QK_WARP_SIZE + qi - 1) / qi;
        ...
        stream->submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1)
                    [[sycl::reqd_sub_group_size(QK_WARP_SIZE)]] {
    ...
    const sycl::range<3> block_dims(1, GGML_SYCL_MMV_Y, QK_WARP_SIZE);
    {
        ...
        stream->submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1)
                    [[sycl::reqd_sub_group_size(QK_WARP_SIZE)]] {
                        mul_mat_vec_q_K<QK_K, QI2_K, block_q2_K,
    ...
    const sycl::range<3> block_dims(1, GGML_SYCL_MMV_Y, QK_WARP_SIZE);
    {
        ...
        stream->submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1)
                    [[sycl::reqd_sub_group_size(QK_WARP_SIZE)]] {
                        mul_mat_vec_q_K<QK_K, QI3_K, block_q3_K,
    ...
    const sycl::range<3> block_dims(1, GGML_SYCL_MMV_Y, QK_WARP_SIZE);
    {
        ...
        stream->submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1)
                    [[sycl::reqd_sub_group_size(QK_WARP_SIZE)]] {
                        mul_mat_vec_q_K<QK_K, QI4_K, block_q4_K,
    ...
    const sycl::range<3> block_dims(1, GGML_SYCL_MMV_Y, QK_WARP_SIZE);
    {
        ...
        stream->submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1)
                    [[sycl::reqd_sub_group_size(QK_WARP_SIZE)]] {
                        mul_mat_vec_q_K<QK_K, QI5_K, block_q5_K,
    ...
    const sycl::range<3> block_dims(1, GGML_SYCL_MMV_Y, QK_WARP_SIZE);
    {
        ...
        stream->submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1)
                    [[sycl::reqd_sub_group_size(QK_WARP_SIZE)]] {
                        mul_mat_vec_q_K<QK_K, QI6_K, block_q6_K,
